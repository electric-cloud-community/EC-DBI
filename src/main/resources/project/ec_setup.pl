my %ExecuteSQL = (
    label       => "DBI - Execute SQL",
    procedure   => "ExecuteQuery",
    description => "Executes SQL against the selected database.",
    category    => "Database"
);

my %BackUpDB = (
    label       => "DBI - BackUp Database",
    procedure   => "BackUpDB",
    description => "Creates a backup for the selected database.",
    category    => "Database"
);

$batch->deleteProperty("/server/ec_customEditors/pickerStep/DBI - Execute SQL");
$batch->deleteProperty("/server/ec_customEditors/pickerStep/DBI - BackUp Database");
$batch->deleteProperty("/server/ec_customEditors/pickerStep/EC-DBI - BackUpDB");
$batch->deleteProperty("/server/ec_customEditors/pickerStep/EC-DBI - ExecuteSQL");

@::createStepPickerSteps = (\%ExecuteSQL, \%BackUpDB);

if ($upgradeAction eq 'upgrade') {
    my $query = $commander->newBatch();
    my $newcfg = $query->getProperty(
        "/plugins/$pluginName/project/Server_cfgs");
    my $oldcfgs = $query->getProperty(
        "/plugins/$otherPluginName/project/Server_cfgs");
    ...
    if ($query->findvalue($newcfg,"code") eq "NoSuchProperty") {
        ...
        if ($query->findvalue($oldcfgs,"code") ne "NoSuchProperty") {
            $batch->clone({
                path => "/plugins/$otherPluginName/project/Server_cfgs",
                cloneName => "/plugins/$pluginName/project/Server_cfgs"
            });
        }
    }
